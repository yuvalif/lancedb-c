...
public:
    static std::shared_ptr<arrow::Schema> create_vector_schema(int dimension) {
        ...
        return arrow::schema({key_field, data_field, metadata_field});
    }
    ...
    static bool add_vectors(LanceDBTable* table,
        ...
        auto record_batch = arrow::RecordBatch::Make(
            schema, keys.size(), {key_array, data_array, metadata_array});
...
ApiResponse PutVectors(const json& request) {
    ...
    std::string error;
    bool success = LanceDBHelper::add_vectors(table, keys, vectors, metadata_list, dimension, error);
